...
export default function VideoEmbed({post,active=true}:{post:Post;active?:boolean}){
 ...
 return <div className={`native-video ${video.orientation==='portrait'?'native-video--portrait':''}`} ref={container}>{playing&&video.kind==='youtube'?<YouTubePlayer video={video} mode={prefs.videoControls} onMode={videoControls=>setPrefs({videoControls})}/>:<div className="native-video-stage">{playing?<video src={video.url} controls autoPlay playsInline preload="metadata"/>:<button className="video-poster" onClick={()=>setPlaying(true)} aria-label={`${video.title} videosunu oynat`}><VideoPoster post={post} className="video-poster-image"/><span className="video-play"><Play fill="currentColor" size={30}/></span><span className="video-length">{video.duration?`${Math.floor(video.duration/60)}:${String(video.duration%60).padStart(2,'0')}`:'Video'}</span></button>}</div>}<div className="video-credit"><span>{video.publisher&&<strong>{video.publisher} · </strong>}{video.language}</span><a href={video.kind==='youtube'?`https://www.youtube.com/watch?v=${video.url}`:video.url} target="_blank" rel="noreferrer">Kaynağında izle <ExternalLink size={13}/></a></div></div>;
