...
export default function VideoEmbed({post,active=true,immersive=false,clean=false}:{post:Post;active?:boolean;immersive?:boolean;clean?:boolean}){
 ...
 useEffect(()=>{const pause=()=>setPlaying(false);window.addEventListener('learnie:pause-videos',pause);return()=>window.removeEventListener('learnie:pause-videos',pause);},[]);
 if(!video)return null;
 return <div className={`native-video ${video.orientation==='portrait'?'native-video--portrait':''} ${immersive?'native-video--immersive':''}`} ref={container}>{playing&&video.kind==='youtube'?<YouTubePlayer video={video} mode={clean?'minimal':prefs.videoControls} clean={clean} immersive={immersive} onMode={videoControls=>setPrefs({videoControls})}/>:<div className="native-video-stage">{playing?<video src={video.url} controls={!clean} autoPlay playsInline preload="metadata"/>:<button className="video-poster" onClick={()=>setPlaying(true)} aria-label={`${video.title} videosunu oynat`}><VideoPoster post={post} className="video-poster-image"/><span className="video-play"><Play fill="currentColor" size={30}/></span><span className="video-length">{video.duration?`${Math.floor(video.duration/60)}:${String(video.duration%60).padStart(2,'0')}`:'Video'}</span></button>}</div>}{!immersive&&<div className="video-credit"><span>{video.publisher&&<strong>{video.publisher} · </strong>}{video.language}</span><a href={video.kind==='youtube'?`https://www.youtube.com/watch?v=${video.url}`:video.url} target="_blank" rel="noreferrer">Kaynağında izle <ExternalLink size={13}/></a></div>}</div>;
