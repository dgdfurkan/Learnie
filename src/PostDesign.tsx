...
export function PostThumbnail({post}:{post:Post}){return <><>{post.display==='video'&&post.video?<img className="thumbnail-video" src={post.video.kind==='youtube'?`https://i.ytimg.com/vi/${post.video.url}/hqdefault.jpg`:post.cover.url} alt="" loading="lazy"/>:<PostArtwork post={post} compact/>}</>{(post.display!=='single')&&<span className="thumbnail-type" aria-hidden="true">{post.display==='video'||post.format==='reel'?<Play size={19} fill="currentColor"/>:<Copy size={17}/>}</span>}</>;}
