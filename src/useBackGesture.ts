...
export function useBackGesture<T extends HTMLElement>(ref:RefObject<T|null>,onBack:()=>void){
 ...
 useEffect(()=>{
  ...
  const down=(e:PointerEvent)=>{if(!e.isPrimary||e.button!==0||(e.target as Element).closest('input,textarea,select'))return;const r=element.getBoundingClientRect();const x=e.clientX-r.left;if(x>30&&x<r.width-30)return;dragged=false;start={x,y:e.clientY,width:r.width,left:r.left,at:Date.now(),pointer:e.pointerId};};
