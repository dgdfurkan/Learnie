export const defaultPreferences = Object.freeze({theme:'system',focus:false,reveal:'word',wordSpeed:180,charSpeed:18,textSize:1,narration:false,music:false,musicVolume:18,savedLayout:'grid',videoControls:'native',videoMuted:false,videoVolume:100,videoPolicy:2,videoCaptions:true,cleanHide:['header','sound','captions','like','comment','share','save','note','more','account','title','caption','source','progress','shade'],voiceURI:'',speechRate:1});
export function normalizePreferences(raw={}) {
 ...
 return {theme:pick('theme',['system','light','dark']),focus:typeof p.focus==='boolean'?p.focus:false,reveal:pick('reveal',['word','char']),wordSpeed:number('wordSpeed',60,360),charSpeed:number('charSpeed',5,40),textSize:number('textSize',.9,1.2),narration:typeof p.narration==='boolean'?p.narration:false,music:typeof p.music==='boolean'?p.music:false,musicVolume:number('musicVolume',0,40),savedLayout:pick('savedLayout',['grid','list']),videoControls:'native',videoMuted:p.videoPolicy===2&&typeof p.videoMuted==='boolean'?p.videoMuted:false,videoPolicy:2,videoCaptions:typeof p.videoCaptions==='boolean'?p.videoCaptions:true,cleanHide:Array.isArray(p.cleanHide)?p.cleanHide.filter(k=>defaultPreferences.cleanHide.includes(k)):[...defaultPreferences.cleanHide],videoVolume:number('videoVolume',0,100),voiceURI:typeof p.voiceURI==='string'&&p.voiceURI.length<=300?p.voiceURI:'',speechRate:number('speechRate',.7,1.3)};
}
