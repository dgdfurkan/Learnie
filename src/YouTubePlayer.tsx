...
export default function YouTubePlayer({video,mode,onMode}:{video:NonNullable<Post['video']>;mode:'minimal'|'native';onMode:(mode:'minimal'|'native')=>void}){
 ...
 return <div className="youtube-shell" ref={shell}><div className="native-video-stage">{mode==='native'?<iframe title={video.title} src={`https://www.youtube-nocookie.com/embed/${video.url}?${new URLSearchParams(Object.entries({...youtubeOptions('native',location.origin),start:Math.floor(resume.current)}).map(([key,value])=>[key,String(value)]))}`} allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin"/>:<div className="youtube-host" ref={host}/>}</div>
  {mode==='minimal'&&<div className="quiet-video-controls" role="group" aria-label="Video denetimleri"><button onClick={play} disabled={!ready} aria-label={state===1?'Videoyu duraklat':state===0?'Videoyu yeniden oynat':'Videoyu oynat'}>{state===1?<Pause size={19}/>:state===0?<RotateCcw size={19}/>:<Play size={19}/>}</button><input type="range" aria-label="Video konumu" min={0} max={duration||1} step={1} value={Math.min(time,duration||1)} disabled={!ready} onChange={e=>{const n=Number(e.target.value);setTime(n);player.current?.seekTo(n,true);}}/><span className="video-clock">{videoTime(time)} / {videoTime(duration)}</span><button onClick={volume} disabled={!ready} aria-label={muted?'Video sesini aç':'Video sesini kapat'}>{muted?<VolumeX size={18}/>:<Volume2 size={18}/>}</button><button onClick={full} aria-label="Videoyu tam ekran aç"><Maximize size={17}/></button></div>}
  <div className="video-player-options"><span role="status">{mode==='native'?'':error||(!ready?'Oynatıcı yükleniyor…':state===3?'Video yükleniyor…':'')}</span><button onClick={()=>onMode(mode==='minimal'?'native':'minimal')}>{mode==='minimal'?'Standart denetimler':'Sade denetimler'}</button></div>
 </div>;
}
